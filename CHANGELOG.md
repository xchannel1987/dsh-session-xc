...
### Chore
- **工程化规范化（无运行时改动，npm 包内容不变）**：
  - `build.ps1` 升级为统一模板：以 `lib/` 为准校验并同步 `src/`；`-NoSync` 只校验、发现漂移即报错退出（供 CI 与发布前把关）；**只清理「当前版本」的同名 tgz**，历史版本一律保留 —— profile 的 `file:` 依赖可能正指向旧版本文件，删掉会打断依赖解析。
  - CI（`.github/workflows/ci.yml`）增加 `src/` ↔ `lib/` 逐字节一致性门禁，并把单元测试步骤改为统一模板（当前无测试文件时自动跳过）。
  - `CLAUDE.md` 更正 `src/`、`lib/` 的权威关系（`lib/` 为权威主本、`src/` 为同源镜像）与构建流程，删除已过期的「无构建步骤」表述。
