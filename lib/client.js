// dsh-session-xc 浏览器端 bundle（手写，无构建步骤）v0.5.3
//
// 功能：
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function computeArchiveData(workspaceSnapshot, sessionsSnapshot) {
        var items = Array.isArray(workspaceSnapshot.items) ? workspaceSnapshot.items : [];
        var archivedSessionIds = Array.isArray(workspaceSnapshot.archivedSessionIds) ? workspaceSnapshot.archivedSessionIds : [];
      ...
      function refresh() {
        try {
          if (workspacesList && typeof workspacesList.getSnapshot === "function") {
            var wsSnapshot = workspacesList.getSnapshot();
            var sessionsSnapshot = null;
            if (workspacesService && workspacesService.sessions && workspacesService.sessions.list) {
              sessionsSnapshot = workspacesService.sessions.list.getSnapshot();
            }
            computeArchiveData(wsSnapshot, sessionsSnapshot);
          }
        } catch (e) { /* ignore */ }
      }

      // v0.5.0: 订阅 DSH 核心的 workspaces.list store
      var workspacesUnsubscribe = null;

      if (workspacesList && typeof workspacesList.subscribe === "function") {
        // 订阅 workspaces 状态变化
        workspacesUnsubscribe = workspacesList.subscribe(function () {
          try {
            var wsSnapshot = workspacesList.getSnapshot();
            // 只有当 phase 为 ready 时才计算，避免使用不完整的数据
            if (wsSnapshot && wsSnapshot.phase === "ready") {
              var sessionsSnapshot = null;
              if (workspacesService && workspacesService.sessions && workspacesService.sessions.list) {
                sessionsSnapshot = workspacesService.sessions.list.getSnapshot();
              }
      ...
      function applyAll() {
        try {
          ...
          for (var i = 0; i < rows.length; i++) {
            ...
            var btn = mount.querySelector("[" + BTN_ATTR + "]");
      ...
      /** 从会话行提取 sessionId */
      function getSessionIdFromRow(sessionRow) {
        // 官方会话行没有直接存储 sessionId，需要通过其他方式获取
        // 尝试从 aria-label 或其他属性获取
        var ariaLabel = sessionRow.getAttribute("aria-label");
        if (ariaLabel && ariaLabel.indexOf("session-") === 0) return ariaLabel;
        // 备选：遍历 workspaceItems 的 sessionIds，匹配标题
        var title = (sessionRow.textContent || "").trim();
        for (var i = 0; i < workspaceItems.length; i++) {
          var ws = workspaceItems[i];
          var ids = Array.isArray(ws.sessionIds) ? ws.sessionIds : [];
          // 这里需要 sessions list 来匹配标题，暂时跳过
        }
        return null;
      }
      ...
      function injectDragListeners() {
        ...
        for (var j = 0; j < sessionRows.length; j++) {
          ...
          sRow.addEventListener("dragstart", function (e) {
            // 从 dataTransfer 获取 sessionId（官方已设置）
          ...
          sRow.addEventListener("dragend", function (e) {
            currentDragSessionId = null;
            currentDragSourceWorkspaceId = null;
            clearDropHighlight();
          });
      ...
      if (typeof document !== "undefined" && typeof window !== "undefined") {
        visibilityHandler = function () { if (!document.hidden) { try { var wsSnapshot = workspacesList && typeof workspacesList.getSnapshot === "function" ? workspacesList.getSnapshot() : null; if (wsSnapshot && wsSnapshot.phase === "ready") { var sessionsSnapshot = workspacesService && workspacesService.sessions && workspacesService.sessions.list ? workspacesService.sessions.list.getSnapshot() : null; computeArchiveData(wsSnapshot, sessionsSnapshot); } } catch (e) { /* ignore */ } } };
        ...
        function refreshFallback() {
          try {
            ...
            res.then(function (r) {
              ...
              computeArchiveData({ items: items, archivedSessionIds: archivedSessionIds, phase: "ready" }, null);
            }).catch(function () { /* 瞬时失败 */ });
          } catch (e) { /* ignore */ }
        }
