// dsh-session-xc 浏览器端 bundle（手写，无构建步骤）v0.2.8
//
// 功能：
...
//   配置："会话增强"设置卡片（settings.plugin.item，mobile-xc 同款 UI）：
    ...
    function apply(ctx) {
      ...
      function mountBtnIcon(btn) {
        ...
        try {
          ...
          if (prim && prim.IconArchiveOutline20 && ReactMod) {
            ...
            if (createRoot) {
              ...
              root.render(ReactMod.createElement(prim.IconArchiveOutline20, { size: 16 }));
              btn._dstkRoot = root;
              return;
            }
            if (RD && typeof RD.render === "function") { RD.render(ReactMod.createElement(prim.IconArchiveOutline20, { size: 16 }), mount); return; }
          }
        } catch (e) { /* 回退 */ }
        ...
        try {
          ...
          for (var i = 0; i < rows.length; i++) {
            ...
            if (!btn) {
              ...
              btn.addEventListener("mouseenter", function () { this.style.color = "var(--dsw-alias-label-primary, #1d2129)"; });
              btn.addEventListener("mouseleave", function () { this.style.color = "var(--dsw-alias-label-tertiary, #8a94a6)"; });
        ...
        try {
          ...
          ctx.effect(function () {
            ...
            styleTag.textContent = [
              ".dsh-sxc-card{border:1px solid var(--dsw-alias-border-l1,#2a3342);border-radius:10px;overflow:hidden;margin:4px 0;background:var(--dsw-alias-bg-layer-2,#1e2430)}",
              ".dsh-sxc-header{display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;padding:10px 12px;background:transparent;border:none;cursor:pointer;color:inherit;text-align:left}",
              ".dsh-sxc-headText{display:flex;flex-direction:column;gap:2px;min-width:0}",
              ".dsh-sxc-name{font-size:14px;line-height:20px;font-weight:600;color:var(--dsw-alias-label-primary,#e2e8f0)}",
              ".dsh-sxc-description{font-size:12px;line-height:16px;color:var(--dsw-alias-label-caption,#94a3b8)}",
              ".dsh-sxc-chevron{flex:none;transition:transform .18s var(--ds-ease-in-out,ease);color:var(--dsw-alias-label-tertiary,#8a94a6)}",
              ".dsh-sxc-chevronOpen{transform:rotate(180deg)}",
              ".dsh-sxc-body{padding:0 12px 10px}",
              ".dsh-sxc-srow{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 0;border-top:1px solid var(--dsw-alias-border-l1,#2a3342);cursor:pointer}",
              ...
              ".dsh-sxc-srow-title{font-size:13px;line-height:18px;color:var(--dsw-alias-label-primary,#e2e8f0);font-weight:500}",
              ".dsh-sxc-srow-hint{font-size:12px;line-height:16px;color:var(--dsw-alias-label-caption,#94a3b8)}",
