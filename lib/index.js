...
async function maybeAutoTitleFirstRound(t, session, sid) {
  ...
  // 设置开关（服务端直读 dsh-settings；触发点与落库点双检查，覆盖运行中关闭的场景）
  if (t.settings) {
    const cfg = t.settings.get(NS);
    if (cfg && cfg.autoTitleFirstRound === false) { note("skip: autoTitleFirstRound disabled"); return; }
  }
  try {
...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(["sessionTitle", "sessions", "llm", "settings"], (t) => {
    const off = ctx.on("session/event", (session, event) => {
      try {
        ...
        if (event && event.type === "user/message" && event.data && event.data.source && event.data.source.kind === "user") {
          ...
          if (t.settings) {
            const cfg = t.settings.get(NS);
            if (cfg && cfg.autoTitleFirstRound === false) return;
          }
          st.scheduled = true;
          const timer = setTimeout(() => {
