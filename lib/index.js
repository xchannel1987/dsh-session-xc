    ...
    c.connection.rpc.handle(
      ...
      async (endpoint, payload) => {
        ...
        if (endpoint === "moveSession") {
          ...
          const oldCwd = registry.sessionPaths.get(sessionId);
        ...
        if (endpoint === "deleteSession") {
          ...
          // 4. 先从 workspace.sessionIds 脱离，避免状态窗口期出现在未分组列表
          for (const ws of registry.list()) {
            if (ws.record.sessionIds.includes(sessionId)) {
              try {
                await ws.mutate(record => ({
                  ...record,
                  sessionIds: record.sessionIds.filter(id => id !== sessionId)
                }));
              } catch (err) {
                console.error("[dsh-session-xc] Failed to detach from workspace:", err);
              }
            }
          }
          ...
          // 7. 最后移除 archivedSessionIds，避免会话在删除完成前被视为普通会话
          const nextState = {
            ...registry.state,
